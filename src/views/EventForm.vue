        ...
        <div class="form">
            <b-form @submit="onSubmit" id="mainForm">
                <b-form-group label="Casa do Evento" label-for="event-place">
                    ...
                    >
                        <option
                            ...
                            :key="place.id"
                            :value="place.id"
                            :disabled="userIsNotAdm(place.id)"
                            >{{ place.name }}
                            {{
                                userIsNotAdm(place.id) ? " - indisponível" : ""
                            }}
                        </option>
                    </b-form-select>
                </b-form-group>
                ...
                <b-form-group
                    ...
                    label-for="event-tickets"
                >
                    <b-form-input
                        type="number"
                        id="event-tickets"
                        v-model="event.tickets"
                        min="1"
                        required
                    ></b-form-input>
                </b-form-group>
...
export default {
    ...
    methods: {
        ...
        userIsNotAdm(place_id) {
            return !this.activeUser.adm.includes(place_id);
        }
    },
    computed: {
        ...mapGetters(["places", "events", "activeUser"]),
        event() {
