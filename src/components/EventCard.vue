<template>
  ...
  <div v-if="event">
    ...
    >
      ...
      <b-card-text :class="[event.tickets > ticketsByEventId(event.id) ? '' : 'disabled']">
      ...
      <b-button
        ...
        variant="success"
        @click="buy(event)"
        :class="[event.tickets > ticketsByEventId(event.id) ? '' : 'disabled']"
      >Comprar</b-button>
      <slot></slot>
    </b-card>
        ...
        <b-list-group>
          ...
          >
            ...
            <div style="display: flex-end">
              <b-badge variant="primary" pill>{{item.ticketAmmount}}</b-badge>
              <b-badge
                variant="danger"
                pill
